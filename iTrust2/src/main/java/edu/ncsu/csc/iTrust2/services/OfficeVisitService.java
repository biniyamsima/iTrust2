...
public class OfficeVisitService extends Service<OfficeVisit, Long> {
    ...
    public OfficeVisit build ( final OfficeVisitForm ovf ) {
        ...
        // Get CPT Codes Forms from OV form, convert each to a CPT Code and set
        // to the
        // office visit.

        final List<CPTCode> cf = ovf.getCPTCodeList(); // was cptcodeform
        if ( cf != null ) {

            ov.setCPTCodeList( cf );
