...
public class APIVaccineVisitController extends APIController {
    ...
    public ResponseEntity createVaccineVisit ( @RequestBody final VaccineVisitForm vvf ) {
        try {
            ...
            // When Visit is created a bill is also created.
            Bill vBill = new Bill();
            vBill.setCodes( request.getCPTList() );
            vBill.setDate( request.getDate().toLocalDate() ); // TODO: Chnage 
            vBill.setHCP(request.getVaccinator().getUsername());
            vBill.setPatient(request.getPatient().getUsername());
            vBill.setStatus(BillStatus.UNPAID);
            billService.save(vBill);
