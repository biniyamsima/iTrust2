...
public class User extends DomainObject {
     ...
     * provided contains either a Patient, Admin, or Billing role, and that role
    ...
    public void setRoles ( final Set<Role> roles ) {
        ...
        /* Patient, admin, billing can't have any other roles */
        if ( ( roles.contains( Role.ROLE_PATIENT ) || roles.contains( Role.ROLE_ADMIN )
                || roles.contains( Role.ROLE_BILLING ) ) && 1 != roles.size() ) {
            throw new IllegalArgumentException(
                    "Tried to create a Patient, Admin, or Billing user with a secondary role. Patient & admin can only have a single role!" );
        }
     ...
     * roles to a patient/admin/biller, or add patient/admin/biller roles to
     * anyone with any existing role(s).
     *
     * @param role
     *            the Role to add
     */
    public void addRole ( final Role role ) {
        ...
        if ( role.equals( Role.ROLE_ADMIN ) || role.equals( Role.ROLE_PATIENT ) || role.equals( Role.ROLE_BILLING ) ) {
            throw new IllegalArgumentException( "Admin, Patient, and Billing roles cannot be added" );
        }
        if ( this.roles.contains( Role.ROLE_ADMIN ) || this.roles.contains( Role.ROLE_PATIENT )
                || this.roles.contains( Role.ROLE_BILLING ) ) {
            throw new IllegalArgumentException(
                    "Admins, Patients, and Billing users cannot have additional roles added" );
        }
        this.roles.add( role );
    }
