...
public class APIOfficeVisitController extends APIController {
    ...
    public ResponseEntity createOfficeVisit ( @RequestBody final OfficeVisitForm visitForm ) {
        try {
            ...
            final List<CPTCode> temp = visitForm.getCPTCodeList();
            final List<CPTCode> codes = new ArrayList<CPTCode>();
            if ( temp != null ) {
                for ( final CPTCode code : temp ) {
                    final CPTCode c = cptCodeService.findById( code.getId() );
                    if ( c != null ) {
                        codes.add( c );
                    }
                }
                ...
                System.out.println( "Not getting codes" );
            }

            for ( final CPTCode code : cptCodeService.findAll() ) {
                System.out.println( code.getId() );
            }

            visit.setCPTCodeList( codes );
            // Create a bill associated with the office visit.
            final Bill ovBill = new Bill();
            ovBill.setCodes( visit.getCPTCodeList() );
            ovBill.setDate( visit.getDate().toLocalDate() ); // TODO: Change. OV has zoned
                                               // date
            ovBill.setHCP( visit.getHcp().getUsername() );
            ovBill.setPatient( visit.getPatient().getUsername() );
            // ovBill.setPayments(new ArrayList<BillPayment>());
            ovBill.setStatus( BillStatus.UNPAID );
            billService.save( ovBill );
