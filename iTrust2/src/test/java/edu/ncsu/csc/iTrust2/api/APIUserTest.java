...
public class APIUserTest {
    ...
    public void testCreateUsers () throws Exception {
        ...
        final UserForm u4 = new UserForm( USER_4, PW, Role.ROLE_BILLING, 1 );
        mvc.perform( MockMvcRequestBuilders.post( "/api/v1/users" ).contentType( MediaType.APPLICATION_JSON )
                .content( TestUtils.asJsonString( u4 ) ) ).andExpect( MockMvcResultMatchers.status().isOk() );

        final User biller = service.findByName( USER_4 );
        Assert.assertNotNull( "The created user should be retrievable from the database", biller );

        Assert.assertEquals( "The retrieved user should be a Personnel", Personnel.class, biller.getClass() );

        Assert.assertEquals( "The retrieved user should have 1 role", 1, biller.getRoles().size() );
    }
