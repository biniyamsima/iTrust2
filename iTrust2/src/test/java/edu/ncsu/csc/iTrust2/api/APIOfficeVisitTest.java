...
public class APIOfficeVisitTest {
    ...
    public void testPreScheduledOfficeVisit () throws Exception {
        ...
        final CPTCodeForm cptCode = new CPTCodeForm();
        cptCode.setCode( "92230" );
        cptCode.setCost( 10.00 );
        cptCode.setDescription( "Test code" );

        cptService.save( cptService.build( cptCode ) );
        final CPTCode code = new CPTCode( cptCode );
    ...
    public void testOfficeVisitAPI () throws Exception {
        ...
        final CPTCodeForm cptCode2 = new CPTCodeForm();
        cptCode2.setCode( "92231" );
        cptCode2.setCost( 20.00 );
        cptCode2.setDescription( "Test code2" );

        cptService.save( cptService.build( cptCode ) );
        cptService.save( cptService.build( cptCode2 ) );

        final CPTCode code1 = new CPTCode( cptCode );
        final CPTCode code2 = new CPTCode( cptCode2 );
        ...
        Assert.assertEquals( 1, billService.count() ); // Test that a bill was
                                                       // also created.
        Assert.assertEquals( billService.findAll().get( 0 ).getHCP(), "hcp" );

        mvc.perform( get( "/api/v1/officevisits" ) ).andExpect( status().isOk() )
                .andExpect( content().contentType( MediaType.APPLICATION_JSON_VALUE ) );

        final OfficeVisit ov = officeVisitService.build( visit );
        assertEquals( ov.getCPTCodeList().size(), 2 );
